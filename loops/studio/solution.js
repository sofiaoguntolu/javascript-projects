...
function mealAssembly(protein, grains, veggies, beverages, desserts, numMeals) {
  ...
  for (i = 0; i < numMeals; ++i){
    let assembled = [];
    for (j = 0; j < 5; ++j){
      assembled.push(pantry[j][i]);
    }
    meals.push(assembled);
  }
  return meals;
}
...
function askForNumber() {
  ...
  while(!goodNum){
    ...
    if (numMeals < 1 || numMeals > 6 || numMeals === NaN){
      console.log("You have to pick a number between 1 and 6.");
    } else{
      goodNum = true;
    }
  }
...
function runProgram() {
  ...
  /// UNCOMMENT the remaining commented lines and change the password1 and password2 strings to ensure your code is doing its job ///
   ...
   let password1 = 'GoCards!';
   let password2 = 'KCChiefs';
